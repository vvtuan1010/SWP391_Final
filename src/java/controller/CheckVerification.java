 ...
 * @author ADMIN
 */
@WebServlet(name="CheckVerification", urlPatterns={"/checkverification"})
public class CheckVerification extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(code == null){
            request.setAttribute("mess1", "Verification code is time out. Please request again!!!");
            request.getRequestDispatcher("VerificationCode.jsp").forward(request, response);
        }
        if(c.equals(code)){
            response.sendRedirect("VerificationPassword.jsp");
        }else{
